                ...
                <nav class="navbar navbar-expand-lg table-bordered menu">
                    <ul class="navbar-nav btn-block">
                      <li class="nav-item link col-md-4">
                        <a class="nav-link link col-md-12" href={{ route('jugador.informacion_jug_club',[$usuario->id_jugador,$club->id_club]) }}>CONFIGURACION <span class="sr-only">(current)</span></a>
                      </li>
                      <li class="nav-item link col-md-4">
                        <a class="nav-link link active col-md-12" href="{{ route('jugador.informacion_jug_participacion',[$usuario->id_jugador,$club->id_club]) }}">PARTICIPACION</a>
                      </li>
                     {{--   <li class="nav-item link col-md-4">
        ...
        <div class="card-body">
           <div class="container table-responsive-xl">
              ...
              <tbody>
                ...
                @foreach ($jug_club as $elemento)
               
                  @foreach ($elemento->selecciones as $item)
                  <tr>
                    <td>
                      {{$i}}
                    </td>
                    <td>
                      {{$item->club_participacion->gestiones->nombre_gestion}}
                    </td>
                    <td>
                      {{$item->club_participacion->disciplina->nombre_disc}}
                    </td>
                    <td>
                      {{$item->club_participacion->disciplina->categoria == 1 ? "Damas": ($item->club_participacion->disciplina->categoria == 2 ? "Varones":"Mixo" )}}
                    </td>
                  </tr>
                  @php
                      $i++;
